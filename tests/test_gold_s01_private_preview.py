...
class T(unittest.TestCase):
 ...
 def test_materialization_exact_hashes(s):
  rr,rh=repo({'r':b'r'});cr=json.dumps(s.r,sort_keys=True).encode();cc,ch=repo({s.r['control']['request_path']:cr});p=copy.deepcopy(s.p);q=copy.deepcopy(s.r);a=p['fixed_production_adapter_path'];m=q['visual']['no_render_manifest']['path'];t=q['timing']['accepted_contract_path'];c=q['captions']['path'];l=p['runtime_lock']['package_lock_path'];pr,ph=repo({a:b'a',m:b'm',t:b't',c:b'c',l:b'lock'})
  q['runner']['sha'],q['control']['sha'],q['production']['sha']=rh,ch,ph;q['visual']['visual_runtime_head']=ph;q['visual']['production_adapter_git_blob_sha']=git(pr,'hash-object',a);q['visual']['no_render_manifest'].update(git_blob_sha=git(pr,'hash-object',m),sha256=G.sha256_file(pr/m));q['timing']['accepted_contract_sha256']=G.sha256_file(pr/t);q['captions']['sha256']=G.sha256_file(pr/c);p['runtime_lock'].update(package_lock_git_blob_sha=git(pr,'hash-object',l),package_lock_sha256=G.sha256_file(pr/l),package_lock_size_bytes=4);f=G.compute_visual_input_fingerprint(q);q['visual']['visual_input_fingerprint']=f;q['visual']['no_render_manifest']['visual_input_fingerprint']=f
  with mock.patch.dict(os.environ,{'GITHUB_ACTIONS':'false'},clear=False):s.assertEqual(G.materialize_and_verify_inputs(q,p,rr,cc,pr)['visual_input_fingerprint'],f)
  (pr/c).write_text('bad');s.err('CHECKOUT_DIRTY',G.materialize_and_verify_inputs,q,p,rr,cc,pr)
