...
fail() {
  echo "gate_id=CONTENT_SEMANTICS_SNAPSHOT_V6_GATE"
  echo "result=FAIL"
  echo "error_class=$1"
  echo "error_code=$2"
...
assert unit13["concept"] == "Следующий блок — кухня разработчика: редактор, терминал, GitHub, Ruby."
assert unit13["success_criterion"] == "Следующий блок — кухня разработчика: редактор, терминал, GitHub, Ruby."
assert unit13["technical_truth_claims"][0]["statement"] == "Следующий блок — кухня разработчика: редактор, терминал, GitHub, Ruby."
...
SOURCE_WORKFLOW_HASH="$(sha256sum "$PRIVATE_DIR/.github/workflows/content-semantics-launch.yml" | awk '{print $1}')"
FACTORY_REQUEST_HASH="$(sha256sum "$ROOT/M1-L01-S02_factory_request_v1.json" | awk '{print $1}')"

cat <<EOF
gate_id=CONTENT_SEMANTICS_SNAPSHOT_V6_GATE
...
canonical_b13_takeaway_preserved=true
...
sha256_source_workflow=$SOURCE_WORKFLOW_HASH
